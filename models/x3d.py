...
class BlockX3D(nn.Module):
    ...
    def __init__(self,
        ...
        ###
        # Default values for updated layers:
        #     Convolution type: Conv3d
        #     Normalization type: BatchNorm3d
        ...
        self.conv1 = nn.Sequential(OrderedDict([('conv', nn.Conv3d(in_channels=inplanes,
                                                                    out_channels=planes,
                                                                    kernel_size=1,
                                                                    stride=1,
                                                                    padding=0,
        ...
        self.conv2 = nn.Sequential(OrderedDict([('conv', nn.Conv3d(in_channels=planes,
                                                                    out_channels=planes,
                                                                    kernel_size=3,
        ...
        self.conv3 = nn.Sequential(OrderedDict([('conv', nn.Conv3d(in_channels=planes,
...
class X3D(nn.Module):
    ...
    def __init__(self,
        ...
        # x3d-s and xs will have a different initial head layer
        ...
        self.conv5 = nn.Sequential(OrderedDict([('conv', nn.Conv3d(
                                                                in_channels=self.feat_dim,
                                                                out_channels=int(self.feat_dim * self.gamma_b),
                                                                kernel_size=1,
                                                                stride=1,
                                                                padding=0,
                                                                bias=False  
                                                                )),
                                                ('bn', nn.BatchNorm3d(int(self.feat_dim * self.gamma_b))),
                                                ('relu', nn.ReLU())])
                                    )
        
        self.feat_dim = int(self.feat_dim * self.gamma_b)
        self.cls_head = X3DHead(in_channels=self.feat_dim,
    ...
    def make_res_layer(self,
        ...
        if spatial_stride != 1 or layer_inplanes != inplanes:
            ...
            downsample = nn.Sequential(OrderedDict([('conv', nn.Conv3d(in_channels=layer_inplanes,
    ...
    def _make_stem_layer(self):
        ...
        self.conv1_s = nn.Sequential(OrderedDict([('conv', nn.Conv3d(in_channels=self.in_channels,
        ...
        initial_stride = 5 if self.model_type not in ['S', 'XS'] else 6
        self.conv1_t = nn.Sequential(OrderedDict([('conv', nn.Conv3d(in_channels=self.base_channels,
                                                  out_channels=self.base_channels,
                                                  kernel_size=(initial_stride, 1, 1),
                                                  stride=(1, 1, 1),
                                                  padding=(2, 0, 0),
                                                  groups=self.base_channels,
                                                  bias=False
                                                  )),
                                                 ('bn', nn.BatchNorm3d(self.base_channels)),
                                                 ('relu', nn.ReLU())])
                                    )
        ...
        if self.zero_init_residual:
            for m in self.modules():
                if isinstance(m, BlockX3D):
                    # Initialize batch norm weights to 0
                    nn.init.constant_(m.conv3[1], 0)
...
class X3DHead(nn.Module):
    ...
    def init_weights(self) -> None:
        ...
        nn.init.normal_(self.fc1, std=self.init_std)
        nn.init.normal_(self.fc2, std=self.init_std)
...
if __name__ == "__main__":
    ...
    ckpt = torch.load('/workspace/pytorch_gpu/sign_language_code/model_ckpts/pretrained/x3d/x3d_m.pth')
    model.load_state_dict(ckpt)
    print('=====Model architecture=====')
    print(model)
    print('==========')
    x = torch.randn(1, 3, 13, 160, 160)
    y = model(x)
    print(y.shape)
